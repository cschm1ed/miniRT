...
int check_triangleIntersect(t_triangle triangle, t_vector intersection_point, t_vector n)
{
	if (_dot(_cross(_subtract(triangle.B, triangle.A), _subtract(intersection_point, triangle.A)), n) >= 0
        && _dot(_cross(_subtract(triangle.C, triangle.B), _subtract(intersection_point, triangle.B)), n) >= 0
        && _dot(_cross(_subtract(triangle.A, triangle.C), _subtract(intersection_point, triangle.C)), n) >= 0)
		return (TRUE);
	return (FALSE);
}
